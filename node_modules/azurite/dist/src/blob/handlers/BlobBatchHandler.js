...
class BlobBatchHandler {
    constructor(accountDataStore, oauth, metadataStore, extentStore, logger, loose) {
        ...
        const subRequestContextMiddleware = (req, res, locals, next) => {
            ...
            (0, blobStorageContext_middleware_1.internnalBlobStorageContextMiddleware)(new BlobStorageContext_1.default(locals, constants_1.DEFAULT_CONTEXT_PATH), req, res, urlbuilder.getHost(), urlbuilder.getPath(), next, true, true, this.loose);
        };
        ...
        for (const subRequest of subRequests) {
            ...
            while (lineIndex < requestLines.length) {
                if (requestLines[lineIndex] === '')
                    break;
                const header = requestLines[lineIndex].split(constants_1.HTTP_HEADER_DELIMITER);
                if (header.length !== 2)
                    throw new Error("Bad Request");
